...
const getLastContactTime = (entry, daysSinceLaunch) => {
  const FREQUENCY_MAP = {
    "60m": 5,
    "40m": 7,
    "30m": 10,
  ...
  for (let x = 0; x < dataTracker.licenseData.length; x++) {
    ...
    if (licekEntry.callSign === cally.toUpperCase() &&
      parseInt(licekEntry.frequency) === FREQUENCY_MAP[bandas]) {
...
// Get styling based on last contact time
const getContactStyle = (ultimo) => {
  let ponecolor = "";
...
const getFreqCell = (balloon) => {

  let uno = balloon.banda || "";
  let dos = balloon.qrpid !== " " ? balloon.qrpid : "";

  if (balloon.tracker === "qrplabs" || balloon.tracker === "dl6ow") {
    return `<td align="center">~${calcfreq(uno, dos)}</td>`;
  } else {
    return `<td align="center">&#1;${calcfreq(uno, dos)}</td>`;
  }
}

const foundCallSignInSearchParams = (balloon) => {
...
const replaceText = (str, searchText, replaceWith) => {
  if (!str) return '';
  return str.toString().replace(new RegExp(searchText, 'g'), replaceWith);
}

const getAdicionalInfo = (balloon) => {

  let adicio = replaceText(balloon.repito, "on", "") + replaceText(balloon.qp, "on", "");

  if (adicio === "wide=" || adicio === "wide=on") {
    adicio = replaceText(balloon.repito, "on", "") + replaceText(balloon.telen, "on", "");
  }
...
const getFrequencyParams = (balloon, index) => {
...
const getRowsTemplate = (balloons) => {
  return balloons.map((balloon, index) => {
    const daysSinceLaunch = getDaysSinceLaunch(balloon.launch);
    const { lastContactTime, lastHeardText, ponerAsterisco } = getLastContactTime(balloon, daysSinceLaunch);
    const contactStyle = getContactStyle(lastContactTime, lastHeardText);
    const freqCell = getFreqCell(balloon);
    const foundCall = foundCallSignInSearchParams(balloon);
    const adicionalInfo = getAdicionalInfo(balloon);
    const freqParams = getFrequencyParams(balloon, index);
    ...
    return (`<tr onmouseout="if(popupwin1){popupwin1.close()}" style="cursor:pointer;" title="Show Emit Frequency Click for Hide/Restore Or Click to Update">
        ...
        <td align="center" style="white-space:nowrap;border-color:#eeeeee;border-width:1px;" title="YYYYMMDDHHMMSS (z)">${formatDate(balloon.launch)}</td>
...
const generateTableTemplate= (balloons) => {
  return `<thead'>
        ...
        ${getRowsTemplate(balloons)}`;
}

window.addEventListener('load', () => {
  window.bj = JSON.parse(dataTracker.jsonArray);

  document.getElementById('balloonsTable').innerHTML = generateTableTemplate(window.dataTracker.balloons);
  document.getElementById('flyingBalloonsActive').innerHTML = `${window.flying} Balloons Active`;
