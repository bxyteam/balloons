class BalloonLinkMenu {
    ...
    constructor() {
        ...
        this.tbanda = [
            ...
            this.mes = ["", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Set", "Oct", "Nov", "Dec"];
    ...
    processBalloonData(entry) {
        ...
        if (launchd1.length >= 14) {
            ...
            const lanzado2 = new Date(`${year}-${month}-${day}T${hour}:${minute}:${second}Z`);
            const now = new Date();
        ...
        enrichedData.forEach((entry, z) => {
            ...
            if (ballooncall.length > 0) {
                ...
                if (
                    ...
                    let comentx = decodeURIComponent(comentm[0] || "").replace(/\+/g, " ");
                    comentx = comentx
                        .replace(/\.\./g, ".<br>")
                        .replace(/"/g, "''")
                        .replace(/\r\n|\r|\n/g, "<br>");
    ...
    processOutput(callsurl, selectedOther, selectedSSID, balloonsurl) {
        ...
        callsurl.forEach(([callSSID, url]) => {
            ...
            const match = callo.toLowerCase() === selectedOther.toLowerCase() && callssid === selectedSSID;
        ...
        if (balloonsurl.length > 11000) {
            ...
            if (pos !== -1) {
                balloonsurl =
                    balloonsurl.slice(0, pos - 12) +
                    "<br>" +
                    balloonsurl.slice(pos - 12);
            }
        }

        // Build JavaScript bcid array
        //const bcidm = `var bcid = [${[...bcidSet].map(c => `"${c}"`).join(",")}];`;
        const bcidm = JSON.parse(`[${[...bcidSet].map(c => `"${c}"`).join(",")}]`);
    ...
    chartMenuTemplate() {
        const ocultar = this.searchParams.tracker === "zachtek1" ? " style='visibility:hidden;line-height:1px;height:1px !important;'" : "";
    ...
    buildBalloonsUrlTemplate() {
        ...
        const output = this.processOutput(result.callsurl, other, SSID, result.balloonsurl);
      ...
      setTimeout(() => {
         const h = document.getElementById("balloonsurl").getBoundingClientRect().height;
