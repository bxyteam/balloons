...
class BalloonLinkMenu {
  ...
  processOutput(callsurl, selectedOther, selectedSSID, balloonsurl) {
    ...
    // Build call list with DOM elements
    ...
    callsurl.forEach(([callSSID, rawUrl]) => {
      ...
      link.title = callSSID;
      link.href = url; // DOM property - no escaping issues
      link.textContent = callSSID;

      if (match) {
        link.style.backgroundColor = "orange";
      }

      callsContainer.appendChild(link);
      callsContainer.appendChild(document.createElement("br"));

      bcidSet.add(callo);
    });

    // Get call list HTML
    const callsll = callsContainer.innerHTML;

    // Limit balloonurl length (keep as is since it's already HTML)
    if (balloonsurl.length > 11000) {
      const midPoint = Math.floor((balloonsurl.length + 4800) / 2);
      const pos = balloonsurl.indexOf("<a title", midPoint);
      if (pos !== -1) {
    ...
    // Create action links container with DOM elements
    const actionsContainer = document.createElement("div");

    // Helper function to create styled links
    const createActionLink = (href, text, title, styles = {}) => {
      const link = document.createElement("a");
      link.href = href;
      link.innerHTML = `<b><u>${text}</u></b>`;
      link.title = title;
      link.target = styles.target || "_self";
    ...
    // Add main action links
    const trackLink = createActionLink(
      `${HOST_URL}/wsprx${qstring}`,
      "TRACK",
      "New using wspr.live",
    );

    const plusLink = createActionLink(
      `${HOST_URL}/wsprfull${qstring}`,
      "[+]",
      "More points but slower",
    );

    const presetsLink = createActionLink(
      `${HOST_URL}/wsprset`,
      "Presets",
      "See these entries&#13 Or Enter New...",
      { target: "_blank" },
    );

    const u4bLink = createActionLink(
      "https://www.qrp-labs.com/tracking.html",
      "U4B",
      "U4B Settings",
      { target: "_blank" },
    );
    ...
    // Append main links
    actionsContainer.appendChild(trackLink);
    actionsContainer.appendChild(document.createTextNode("\u00A0")); // &nbsp;
    actionsContainer.appendChild(plusLink);
    actionsContainer.appendChild(document.createTextNode("\u00A0"));
    actionsContainer.appendChild(presetsLink);
    actionsContainer.appendChild(document.createTextNode("\u00A0\u00A0"));
    actionsContainer.appendChild(u4bLink);

    // Add Charts link if selectedSSID exists
    if (selectedSSID) {
      const chartsLink = createActionLink(
        `${HOST_URL}/balloonchart?callsign=${selectedOther}-${selectedSSID}&flights=0&grafico=height%20m`,
        "Charts",
        "Plot Graphs if data available on aprs.fi",
        { target: "_blank" },
      );
      actionsContainer.appendChild(document.createTextNode("\u00A0\u00A0"));
      actionsContainer.appendChild(chartsLink);
    }

    // Add remaining links
    const dxLink = createActionLink(
      `${HOST_URL}/dx?call=${selectedOther}&band=${didBand}&bs=B`,
      "DX",
      "WSPR DX Reports",
      { target: "_blank" },
    );

    const satLink = createActionLink(
      "https://satellites.browxy.com/pass",
      "Sat",
      "Track Satellites",
      { target: "_blank" },
    );

    const usersLink = createActionLink(
      `${HOST_URL}/wspruser`,
      "Users",
      "Last Uses",
      { target: "_blank" },
    ...
    actionsContainer.appendChild(document.createTextNode("\u00A0\u00A0"));
    actionsContainer.appendChild(dxLink);
    actionsContainer.appendChild(document.createTextNode("\u00A0\u00A0"));
    actionsContainer.appendChild(satLink);
    actionsContainer.appendChild(document.createTextNode("\u00A0\u00A0"));
    actionsContainer.appendChild(usersLink);

    // Get the HTML for the actions
    const actionsHTML = actionsContainer.innerHTML;

    // Combine with existing balloon HTML
    balloonsurl += actionsHTML + "</span></span></center>";

    // Create calink with DOM elements
    const calinkContainer = document.createElement("center");
    ...
    span.style.cssText =
      "font-size:11px;line-height:10px;font-weight:bold;text-align:center";

    const innerSpan = document.createElement("span");
    innerSpan.style.cssText = "font-size:12px;line-height:11px;";
    innerSpan.textContent = "Browse";

    const hr1 = document.createElement("hr");
    hr1.width = "60px";
    hr1.style.cssText = "color:#ffffff;margin-top:0px;margin-bottom:0px;";

    // Create calink navigation links
    const createNavLink = (href, text, title, target = "_self") => {
      const link = document.createElement("a");
    ...
    const anyLink = createNavLink(
      `${HOST_URL}/wsprx${qstring}`,
      "<u><i>ANY</i></u>",
      "New using wspr.live",
    );
    const presetsNavLink = createNavLink(
      `${HOST_URL}/wsprset`,
      "<u>Presets</u>",
      "See these entries&#13 Or Enter New...",
      "_blank",
    );
    const u4bNavLink = createNavLink(
      "https://www.qrp-labs.com/tracking.html",
      "<u><i>U4B</i></u>",
      "U4B Settings",
      "_blank",
    );
    const chartsNavLink = createNavLink(
      `${HOST_URL}/balloonchart?callsign=${selectedOther}-${selectedSSID}&flights=0&grafico=height%20m`,
      "<u><i>Charts</i></u>",
      "Plot Graphs if data available on aprs.fi",
      "_blank",
    );
    const dxNavLink = createNavLink(
      `${HOST_URL}/dx?call=${selectedOther}&band=${didBand}&bs=B`,
      "<u><i>DX-Prop</i></u>",
      "WSPR DX Reports",
      "_blank",
    );
    const satNavLink = createNavLink(
      "https://satellites.browxy.com/pass",
      "<u><i>Satellite</i></u>",
      "Track Satellites",
      "_blank",
    );
    const usageNavLink = createNavLink(
      `${HOST_URL}/wspruser`,
      "<u><i>Usage</i></u>",
      "Last Uses",
      "_blank",
    ...
    hr2.width = "60px";
    hr2.style.cssText = "color:#ffffff;margin-top:0px;margin-bottom:0px;";

    const hr3 = document.createElement("hr");
    hr3.width = "60px";
    hr3.style.cssText = "color:#ffffff;margin-top:1px;margin-bottom:0px;";

    const entriesText = document.createElement("i");
    entriesText.textContent = `${callsurl.length} Entries`;

    // Assemble the navigation structure
    span.appendChild(innerSpan);
    span.appendChild(document.createElement("br"));
    span.appendChild(hr1);
    span.appendChild(anyLink);
    span.appendChild(document.createElement("br"));
    span.appendChild(presetsNavLink);
    span.appendChild(document.createElement("br"));
    span.appendChild(u4bNavLink);
    span.appendChild(document.createElement("br"));
    span.appendChild(chartsNavLink);
    span.appendChild(document.createElement("br"));
    span.appendChild(dxNavLink);
    span.appendChild(document.createElement("br"));
    span.appendChild(satNavLink);
    span.appendChild(document.createElement("br"));
    span.appendChild(usageNavLink);
    span.appendChild(hr2);

    // Add calls list (using the DOM-generated HTML)
    const callsDiv = document.createElement("div");
    callsDiv.innerHTML = callsll;
    span.appendChild(callsDiv);

    span.appendChild(hr3);
    span.appendChild(entriesText);

    td.appendChild(span);
    tr.appendChild(td);
    table.appendChild(tr);
    calinkContainer.appendChild(table);

    const calink = calinkContainer.outerHTML;
...
*/
  ...
  buildBalloonsUrlTemplate() {
    ...
    spanOuter.style.cssText =
      "font-family:Arial Narrow,Tahoma,Arial;font-size:10px;font-stretch:condensed;font-weight:normal;line-height:11px;white-space:nowrap;";

    const centerInner = document.createElement("center");
    const spanInner = document.createElement("span");
    spanInner.style.cssText =
      "display:flex; flex-direction: row; gap: 3px; align-items: center; justify-content: center;flex-wrap: wrap;padding: 5px;";

    spanInner.appendChild(document.createTextNode("Flights "));
    spanInner.innerHTML += output.balloonsurl; // This part contains the action buttons

    centerInner.appendChild(spanInner);
    spanOuter.appendChild(centerInner);
    centerDiv.appendChild(spanOuter);
