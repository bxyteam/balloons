...
function ponermapa(locator, licencia) {
  ...
  for (i = 0; i < locations.length; i++) {
    if (
      ...
      if (i < 6 && locations.length > 5) {
        if (hide) {
          icono1 = "icon/none.png";
          document.getElementById("estaciones").innerHTML = "";
        } else {
          icono1 = "icon/yellow-dot.png";
  ...
  estima =
    ...
    dhoras +
    "h" +
    "<br>&nbsp;<a href=locator1.gif target='_blank'><u>Use SNR +precision</u></a>";
...
function noupload() {
  ...
  codata =
    ...
    trayecto[0][0] +
    "<br><br>Keep UPLOADS <img src=mark.gif border=0> Marked<\/center>";
  if (gqs("SSID") != "") {
...
if (gqs("tracker") == "traquito" || gqs("tracker") == "qrplabs") {
  var lofreq = "<%=fcentral%>" * 1 - 20;
  var hifreq = "<%=fcentral%>" * 1 + 20;
} else {
  var lofreq = "<%=fcentral%>" * 1 - 100;
  var hifreq = "<%=fcentral%>" * 1 + 100;
}
if (gqs("wide") == "on") {
  var lofreq = "<%=fcentral%>" * 1 - 100;
  var hifreq = "<%=fcentral%>" * 1 + 100;
...
function carga() {
  ...
  if ('<%=Request("other")%>' != "") {
    document.getElementById("other").value =
      '<%=replace(left(Request("other"),9),",","",1,9)%>';
  }
  if ('<%=Request("balloonid")%>' != "") {
    document.formu.balloonid.value = '<%=Request("balloonid")%>';
    formu.balloonid.value = '<%=Request("balloonid")%>';
  }
  if ('<%=Request("callsign")%>' != "") {
    document.formu.other.value = '<%=Request("callsign")%>';
  }
  if ('<%=Request("banda")%>' != "") {
    document.formu.banda.value = '<%=Request("banda")%>';
  }
  if ('<%=Request("tracker")%>' != "") {
    document.formu.tracker.value = '<%=Request("tracker")%>';
  }
  if ('<%=Request("detail")%>' != "") {
...
function setid() {
  ...
  sleep(100);
  document.location.href = querystring;
  sleep(100); // document.location.href = querystring;
  //    document.location.reload();
}
function solarflux() {
